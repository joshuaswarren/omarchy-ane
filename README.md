...
| M2 Max | T6021 | H14J | `apple,t6021-ane` | recognized-blocked | board overlay authored + installed (`ane/t6021-j414c-ane.dts`, boots healthy); bind runbook run 2026-09-18 with netconsole+ramoops: genpd raise (6 domains) and all six SET-window reads pass, then the first engine read `0x285c2400c` (TM_TQ_EN @ engine `0x285c04000+0x20000`) external-aborts the machine — proven twice, kill named at register level (ane-linux-experiments `receipts/2026-09-18-t6021-overlay-abort.md`) | **The engine-internal TM/TQ layout within ADT range0 and the true SET-block location** — no capture source exists (macOS exposes no engine sub-window; the m1n1 constants never ran ANE on T6021). Unlock: an m1n1 ANE probe walk on T6021 silicon, or a macOS ANE-runtime capture surfacing the engine window. Any other offset is a guess that will hard-reset the machine |
