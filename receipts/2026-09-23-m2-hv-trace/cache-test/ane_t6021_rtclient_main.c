...
static bool fw_asc_reset;
module_param(fw_asc_reset, bool, 0444);
MODULE_PARM_DESC(fw_asc_reset,
		 "fw_cache_test only: run the Asahi ISP coproc reset (EDPRCR, fabric, IRQ masks, WFI poll) before release. Touches CoreSight.");
...
/* isp_reset_coproc, drivers/media/platform/apple/isp/isp-fw.c:215. The
 * ANE engine window has the same coproc layout. EDPRCR sits in the
 * CoreSight block; log before each write so a wedge names its step. */
static int ane_asc_reset(void __iomem *eng)
{
	static const u32 fabric[] = { 0x738, 0x798, 0x7f8, 0x858 };
	unsigned int i;
	u32 v;

	pr_emerg("asc-reset: EDPRCR 0x1010310 <- 2\n");
	writel(0x2, eng + 0x1010310);
	pr_emerg("asc-reset: fabric <- 0xff00ff\n");
	for (i = 0; i < ARRAY_SIZE(fabric); i++)
		writel(0xff00ff, eng + fabric[i]);
	pr_emerg("asc-reset: IRQ_MASK 0x1400a00..a14 <- ~0\n");
	for (i = 0; i < 6; i++)
		writel(0xffffffff, eng + 0x1400a00 + 4 * i);
	for (i = 0; i < 128; i++) {
		v = readl(eng + 0x818);
		if (!v)
			break;
	}
	pr_emerg("asc-reset: 0x818=%08x after %u reads\n", v, i);
	for (i = 0; i < 128; i++) {
		v = readl(eng + 0x81c);
		if (!v)
			break;
	}
	pr_emerg("asc-reset: 0x81c=%08x after %u reads\n", v, i);
	for (i = 0; i < 1000; i++) {
		v = readl(eng + ANE_ASC_CPU_STATUS);
		if (v & 0x3)
			break;
		mdelay(1);
	}
	pr_emerg("asc-reset: STATUS=%08x after %u ms\n", v, i);
	if (!(v & 0x3)) {
		pr_err("asc-reset: coproc not in WFI, no release\n");
		return -ENODEV;
	}
	return 0;
}
...
static int ane_cache_test(void)
{
	...
	void __iomem *pmgr, *eng, *dart, *pgd = NULL, *l2 = NULL;
	...
	for (n = 0; n < 60; n++) {
		...
		if ((n % 10) == 0 || scratch || recv)
			pr_emerg("cache-test: t=%u scratch7=%08x i2a=%08x recv0=%016llx status=%08x\n",
				 n, scratch, i2a, recv,
