...
class ProxyUtils(Reloadable):
    ...
    def compressed_writemem(self, dest, data, progress=None):
        if not len(data):
            return
        extents = [(913950004, 3481), (913956521, 5019)]
        total = 34816000
        comp_size = 34812701
        # nvme_read BEFORE the first iface.writemem: the UART proxy's
        # exception-guard probe writes (write8 start/end) appear to leave
        # the memwrite path in a state that reads zero after. In the
        # load path this is the first writemem of the session, since
        # trace_range only maps and hv_init maps without writing.
        buf = self.heap.memalign(4096, total)
        off = 0
        assert self.proxy.nvme_init()
        for lba, count in extents:
            for b in range(count):
                assert self.proxy.nvme_read(1, lba + b, buf + off)
                off += 4096
                if progress and (off // 4096) % 512 == 0:
                    sys.stdout.write(".")
                    sys.stdout.flush()
        if progress:
            print()
        self.proxy.nvme_shutdown()
        timeout = self.iface.dev.timeout
        self.iface.dev.timeout = None
        try:
            n = self.proxy.gzdec(buf, comp_size, dest, len(data))
        finally:
