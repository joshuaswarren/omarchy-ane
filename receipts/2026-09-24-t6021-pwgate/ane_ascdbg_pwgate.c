...
#define PMGR_BASE	0x28e080000ull
#define PMGR_SPAN	0x20000
...
/* EDPRSR.OSLK: DBGDTRRX with the OS lock set hard-reset a T6001. */
static bool dtr_locked(void)
{
	return readl_relaxed(g.engine + 0x1010314) & BIT(5);
}
...
static int check_off(unsigned long off, unsigned int width)
{
	...
	if (off < 0x10100a0 && off + width > 0x1010080 && dtr_locked()) {
		res("EPERM off %#lx: EDPRSR.OSLK set\n", off);
		return -EPERM;
	}
	return 0;
}
...
static int run_cmd(char *line)
{
	...
	if (!strcmp(op, "r32")) {
		...
		res("r64 %#lx = %016llx\n", a[0], v);
	} else if (!strcmp(op, "pr32") || !strcmp(op, "pw32")) {
		/* pmgr/set/PWGATE block 0x28e080000..0x28e0a0000, offsets
		 * relative to 0x28e080000. */
		if (a[0] >= PMGR_SPAN || (a[0] & 3) || !g.pmgr) {
			res("EINVAL pmgr off %#lx\n", a[0]);
			return -EINVAL;
		}
		if (op[1] == 'w') {
			if (n < 3)
				return -EINVAL;
			writel((u32)a[1], g.pmgr + a[0]);
			res("pw32 %#lx <= %08x\n", a[0], (u32)a[1]);
		} else {
			res("pr32 %#lx = %08x\n", a[0], readl(g.pmgr + a[0]));
		}
...
/* T6021: the ane device is unbound and its runtime-PM raise hung a
 * bind on this box. Gate on the pmgr ane_cpu word instead; genpd has
 * the islands on. */
static int ane_ascdbg_islands_on(void)
{
	u32 ps = readl(g.pmgr + 0x2e0);

	pr_info("ane_ascdbg: ane_cpu ps=%08x\n", ps);
	return ((ps >> 4) & 0xf) == 0xf ? 0 : -EPROBE_DEFER;
}
...
static int __init ane_ascdbg_init(void)
{
	...
	found = bus_find_device_by_name(&platform_bus_type, NULL, "284000000.ane");
	if (!found) {
		pr_err("ane_ascdbg: no 284000000.ane device\n");
		return -ENODEV;
	}
	g.pdev = to_platform_device(found);
	mutex_init(&g.lock);

	g.pmgr = ioremap_np(PMGR_BASE, PMGR_SPAN);
	if (!g.pmgr) {
		ret = -ENOMEM;
		goto err;
	}
	ret = ane_ascdbg_islands_on();
	if (ret) {
		pr_err("ane_ascdbg: ANE islands not on: %pe\n", ERR_PTR(ret));
		goto err;
	}
