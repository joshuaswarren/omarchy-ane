...
int main(void)
{
	 ...
	 * zeroed block; publication stays fenced. ---- */
	{
		static const struct ane_t6021_init_sources src = {
			...
			.cfg_size = 0x1a4000, /* ANE_FW_BLOB_SIZE */
		...
		0x00, 0x40, 0x1a, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xc0, 0xe5, 0x0f, 0x00, 0x00, 0x00, 0x00,
		...
		check(rd_le64(buf) == src.fw_dva &&
		      ...
		      rd_le64(buf + 0x10) == 0x1a4000ULL &&
		      rd_le64(buf + 0x18) == 0x0fe5c000ULL &&
		...
		check(rd_le64(leak) == 0 && rd_le64(leak + 0x08) == 0 &&
		      rd_le64(leak + 0x10) == 0x1a4000ULL &&
		      rd_le64(leak + 0x18) == 0x0fe5c000ULL,
