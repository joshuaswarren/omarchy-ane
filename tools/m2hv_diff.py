...
# --------------------------------------------------------------- replay
# Every macOS write to an ANE-relevant register, uncollapsed, in trace
# order, for an observer/driver to apply verbatim before RUN. Flags mark
# the writes a Linux applier must treat specially; 'linux' marks the
# ones the Linux baseline already makes with the same value.
REPLAY_FLAGS = (
    (1, "hook", "via m1n1 PMGR HACK hook: hardware received only bits [9:0] of "
                "(v|0xf)&~0x400 (hv/__init__.py wh); order vs MMIO approximate"),
    (2, "ps-off", "ANE-chain ps word written with TARGET=0: kernel-context "
                  "ps power-cycle is the s24 fatal class on this box"),
    (4, "set-win", "ane0 SET window (pmgr+0xc000): direct SET writes "
                   "external-abort the SoC from Linux (repo rule)"),
    (8, "linux-same", "the Linux baseline already makes this write (same value "
                      "under the compared mask)"),
)
LINUX_TAG = {"MATCH": "same", "MATCH-C": "same", "VALUE": "diff",
             "MISSING": "none", "GATED": "gated", "OBS": "obs"}


def replay_rows(evs, base):
    """Per-event (uncollapsed) diff rows for ANE-relevant writes."""
    rows = [Row(e, 1, e, 1) for e in evs]
    res, _ = diff(rows, base)
    out = []
    for r in res:
        e = r.row.ev
        if not r.rel or e.rw != "W":
            continue
        f = 0
        if e.dev == HOOK_DEV:
            f |= 1
        if r.name.startswith("ps_") and (e.val & 0xF) == 0:
            f |= 2
        if r.name.startswith("ane_SET"):
            f |= 4
        if r.status in ("MATCH", "MATCH-C"):
            f |= 8
        out.append((e, r, f))
    return out


def write_replay(path, trace_path, evs, base, c_path=None):
    rows = replay_rows(evs, base)
    names = {b: n for b, n, _ in REPLAY_FLAGS}
    fl = lambda f: ",".join(n for b, n, _ in REPLAY_FLAGS if f & b) or "-"
    with open(path, "w") as fp:
        fp.write(f"# m2hv replay: every macOS write to an ANE engine/DART/pmgr register\n"
                 f"# in {os.path.basename(trace_path)}, trace order, uncollapsed "
                 f"({len(rows)} writes).\n# Generated by tools/m2hv_diff.py --replay; "
                 f"baseline {os.path.basename(BASELINE)}.\n#\n"
                 "# evt W<bits> addr value linux flags region register [decode]\n"
                 "#   linux  same = Linux baseline makes this write (same value under mask)\n"
                 "#          diff = Linux writes this register with another value\n"
                 "#          none = Linux never writes this register\n"
                 "#          gated/obs = only a gated Linux path / the ane_obs observer does\n"
                 "#   flags  " + "; ".join(f"{n} = {d}" for _, n, d in REPLAY_FLAGS[:3]) + "\n")
        for e, r, f in rows:
            dec = decode(r.name, e.val)
            fp.write(f"{e.idx:<5} W{8 * e.w:<3} {e.addr:#011x} {fmt_val(e.w, e.val):<18} "
                     f"{LINUX_TAG[r.status]:<5} {fl(f & 7):<12} {r.region:<12} "
                     f"{r.name}{'  # ' + dec if dec else ''}\n")
    if c_path:
        with open(c_path, "w") as fp:
            fp.write(f"/* generated by tools/m2hv_diff.py --replay-c from "
                     f"{os.path.basename(trace_path)}: {len(rows)} macOS writes to ANE "
                     "engine/DART/pmgr registers, trace order, uncollapsed. */\n")
            for b, n, d in REPLAY_FLAGS:
                fp.write(f"#define M2HV_F_{n.upper().replace('-', '_'):<11} {b:#x} /* {d} */\n")
            fp.write("struct m2hv_replay_w { u64 addr; u64 val; u32 evt; u8 width; u8 flags; };\n"
                     "static const struct m2hv_replay_w m2hv_replay[] = {\n")
            for e, r, f in rows:
                fp.write(f"\t{{ {e.addr:#x}ull, {e.val:#x}ull, {e.idx}, {e.w}, {f:#x} }},"
                         f" /* {r.name} {LINUX_TAG[r.status]} */\n")
            fp.write("};\n")
    same = sum(1 for _, _, f in rows if f & 8)
    print(f"replay: {len(rows)} writes -> {path}" + (f" + {c_path}" if c_path else "")
          + f"; {same} already made by Linux, "
          f"{sum(1 for _, _, f in rows if f & 2)} ps-off, "
          f"{sum(1 for _, _, f in rows if f & 4)} set-win, "
          f"{sum(1 for _, _, f in rows if f & 1)} hook")
...
def main():
    ...
    ap.add_argument("--replay", metavar="OUT",
                    help="write the uncollapsed ANE write list (replay format) to OUT")
    ap.add_argument("--replay-c", metavar="OUT_H",
                    help="with --replay: also emit a C table of the same writes")
    a = ap.parse_args()
    if a.selftest:
        return selftest()
    if a.linux:
    ...
    path = pick_trace(a.trace)
    if a.replay:
        lines = open(path, errors="replace").read().splitlines()
        write_replay(a.replay, path, parse_trace(lines), load_baseline(a.baseline),
                     a.replay_c)
        return 0
    return report(path, a.all, a.context, a.baseline)
